...
<div class="form-container">
    <form action="{{ $bannerImgBySlug ? route('show.setting-banner-service.update', $bannerImgBySlug->id) : route('show.setting-banner-service.save')}}" method="post" enctype="multipart/form-data">
        @csrf
        ...
        <div class="row mb-3">
            {{-- <div class="col-md-8">
                ...
                <input type="text" class="form-control @error('link') is-invalid @enderror" id="link" name="link" 
                    value="" 
                    placeholder="Masukkan link banner" autofocus>
                ...
                <label for="image">Gambar Banner</label>
                    <div class="avatar-upload">
                        <div>
                            ...
                            <label for="bannerImg"></label>
                            @if($bannerImgBySlug)
                                <input type="hidden" name="oldbannerImg" value="{{ $bannerImgBySlug->bannerImg }}">
                            @endif
                        </div>
                        <div class="image-preview" id="imagePreview">
                            <img src="{{ $imagePath }}" alt="Image Preview" id="imagePreviewImg" style="{{ $bannerImgBySlug ? 'display: block;' : 'display: none;' }}">
                        </div>
                    </div>
                    @error('bannerImg')
                        <small class="text-danger">{{ $message }}</small>
        ...
        <div class="d-flex justify-content-between">
            <a href="{{ route('show.setting-banner-service') }}" class="btn btn-danger">Batal</a>
