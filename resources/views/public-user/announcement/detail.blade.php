...
@section('content')
    ...
    <main class="mx-auto w-[90%] pt-[6.5rem] md:pt-[7rem]">
        <div class="grid grid-cols-1 lg:grid-cols-[70%_30%] gap-5 p-3 w-full mx-auto lg:px-10 lg:py-4 mt-4 shadow-lg bg-white dark:bg-gray-900 rounded-lg">
            ...
            <div class="row justify-content-center">
                <div class="col-lg-8 p-4 shadow-sm rounded-md">
                    ...
                    <div class="text-center mt-5 mb-5">
                        ...
                        <a href="https://twitter.com/intent/tweet?text=<?= urlencode('Check out this announcement!') ?>&url=<?= urlencode('Yoi Guys') ?>" 
                        target="_blank" 
                        ...
                        <a href="https://api.whatsapp.com/send?text=<?= urlencode('Check out this announcement: ' . 'Yoi Guys') ?>" 
                        target="_blank" 
                    ...
                    <div class="mb-3 w-full h-auto bg-black flex items-center justify-center">
                        <img src="{{ asset('storage/images/publicImg/announcement/announcementImg/'. $dA->image) }}" 
                             alt="{{ $dA->title }}" 
                    ...
                    <div class="container px-20 mx-auto lg:px-60 md:px-30 mb-10 mt-10">
                        ...
                        <div class="text-x" style="font-family: 'Arial', sans-serif;">
                            ...
                            <div class="text-center text-sky-950 mt-5 mb-5">
                                    ...
                                    <a href="https://twitter.com/intent/tweet?text=<?= urlencode('Check out this announcement!') ?>&url=<?= urlencode('current_url()') ?>" 
                                    target="_blank" 
                                    title="Share on Twitter" 
                                    class="inline-flex items-center justify-center w-10 h-10 rounded-full bg-gray-200 hover:bg-gray-300">
                                        <i class="fa-brands fa-x-twitter"></i>
                                    </a>
                                    <a href="https://api.whatsapp.com/send?text=<?= urlencode('Check out this announcement: ' . 'current_url()') ?>" 
                                    target="_blank" 
                                    title="Share on WhatsApp" 
                                    class="inline-flex items-center justify-center w-10 h-10 rounded-full bg-gray-200 hover:bg-gray-300">
                                        <i class="fa-brands fa-whatsapp"></i>
                ...
                <div class="mt-6">
                    <div class="p-[2px] inline-block w-full h-10 mb-4">
                        <div class="p-2 bg-sky-950 dark:bg-gray-700 w-full h-full flex justify-between items-center rounded-md">
                            <h2 class="text-lg text-white dark:text-gray-200 font-bold">Pengumuman Terbaru &gt;</h2>
                        </div>
                    </div>
                    <div class="gap-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 w-full flex justify-center md:justify-between lg:justify-between overflow-hidden">
                        ...
                        <div class="relative w-full h-52 rounded-xl overflow-hidden shadow-lg">
                            <img src="{{ asset('storage/images/publicImg/announcement/announcementImg/'. $announcement->image) }}" alt="Announcement Image" class="w-full flex-shrink-0 object-cover h-64">
                            <div class="absolute inset-0 bg-black bg-opacity-50 flex flex-col justify-end p-4">
                              <h2 class="text-white text-xl font-bold">{{ \Illuminate\Support\Str::limit($announcement->title, 30) }}</h2>
                              <p class="text-white text-sm">{{ \Carbon\Carbon::parse($announcement->created_at)->diffForHumans() }}</p>
                            </div>
                        </div>
                        </a>
                        @endforeach
            ...
            <!-- Kolom 2-->
            <div class="row">
                <section class="sticky top-[7rem] rounded-md mt-3 pb-4 bg-gray-300 dark:bg-gray-800">
                    ...
                    <div class="mt-2 space-y-4 lg:px-2 max-h-[70vh] overflow-y-auto">
                        @foreach ($popularAnnouncement as $announcement)     
                        <a href="{{ route('show.announcement.detail', $announcement->slug) }}" class="block">
                            <div class="group bg-white dark:bg-gray-700 dark:text-gray-300 p-4 shadow-md mt-2 rounded-lg">
                                <p class="group-hover:text-blue-600">{{ \Illuminate\Support\Str::limit($announcement->title, 25) }}</p>
                                <div class="flex mt-2">
                                    <small class="text-gray-500 ml-2">
                                        {{ \Carbon\Carbon::parse($announcement->created_at)->translatedFormat('l, d F Y') }}
                                    </small>
                                </div>
                            </div>
                        </a>
